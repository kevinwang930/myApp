...
export async function getDataVersion() {
    const results = await sequelize.query('PRAGMA data_version', {
        type: QueryTypes.SELECT,
    })
...
export async function getSqliteVersion() {
    return await sequelize.query('select sqlite_version() as version', {
        type: QueryTypes.SELECT,
    })
}
...
export async function getProductIdsAndOrderIdsBySupplierId(id) {
    ...
    let productIds_string = await selectEntry(
        'products',
        `group_concat(id)`,
        `supplierId = ${id}`
    )
    let productIds = productIds_string
        .split(',')
        .map((idString) => parseInt(idString))
    let orderIds_string = await selectEntry(
        'orders',
        `group_concat(id)`,
        `supplierId = ${id}`
    )
    let orderIds = orderIds_string
        .split(',')
        .map((idString) => parseInt(idString))
...
export async function getSuppliers() {
    try {
        let suppliers = await sequelize.query(
...
export const getOrderItems = async () => {
    try {
        let orderItems = await sequelize.query(
...
export async function getProducts() {
    try {
        let products = await selectMultipleEntries(
            'products',
