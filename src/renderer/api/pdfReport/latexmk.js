...
async function texgen_async(templatePath, texPath, context) {
    ...
    let texData = await mustache.render(templateString, context)
    await writeFile(texPath, texData)

}

export async function pdfgen_latexmk(templatePath, texPath, reportPath, context) {
    ...
    execFile('latexmk', ['-aux-directory=output/pdfReport/auxilary', '-outdir=output', '-pdf', '-quiet', '-rc-report-', '-rules-', texPath], async (err, stdout, stderr) => {
        if (err) {
            message.error({
                content: err.message,
                key: "supplierReportError"
