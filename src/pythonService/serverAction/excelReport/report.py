...
class OrderExcelReport_win32com(OrderExcelReportBase):
    ...
    def writeToExcel(self):
        ...
        self.xlApp = importlib.import_module(
            ".win32com_interface", package="dbexcel"
        ).xl
    ...
    def writeSupplier(self):
        ...
        if (cellphone := supplier.cellphone) and (telephone := supplier.telephone):
            self.worksheet.Range(self.supplierPhonePosition).Value = (
                cellphone + " / " + telephone
            )
        elif cellphone:
    ...
    def writeOrderItems(self):
        ...
        self.worksheet.Range(
            self.worksheet.Cells(row, column), self.worksheet.Cells(row, column + 5)
        ).Value = self.orderItemHead
        for item in self.orderItems:
            row = row + 1
            self.worksheet.Cells(row, column).NumberFormat = "@"
            self.worksheet.Range(
                self.worksheet.Cells(row, column), self.worksheet.Cells(row, column + 4)
            ).Value = item
            priceAddress = self.worksheet.Cells(row, column + 3).GetAddress(
                RowAbsolute=False, ColumnAbsolute=False
            )
            quantityAddress = self.worksheet.Cells(row, column + 4).GetAddress(
                RowAbsolute=False, ColumnAbsolute=False
            )
            self.worksheet.Cells(
                row, column + 5
            ).Formula = f"={priceAddress}*{quantityAddress}"
...
class OrderExcelReport_openpyxl(OrderExcelReportBase):
    ...
    def writeToExcel(self):
        self.templatePath = getOrderExcelTemplatePath()
        if not self.templatePath:
            return False, f"excel订单模板不存在"
        outputDir = getOutputPath()
        if not outputDir:
            return False, f"无法确定输出目录"
        self.destFile = os.path.join(outputDir, f"订单-{self.orderNo}.xlsx")
    ...
    def writeSupplier(self):
        ...
        if (cellphone := supplier["cellphone"]) and (
            telephone := supplier["telephone"]
        ):
            self.worksheet[self.supplierPhonePosition].value = (
                cellphone + " / " + telephone
            )
        elif cellphone:
    ...
    def writeOrderItems(self):
        ...
        for item in self.orderItems:
            ...
            self.worksheet.cell(
                row, column + 5
            ).value = f"={priceAddress}*{quantityAddress}"

    def saveWorkbook(self):
        dest_file = str(self.destFile)
        try:
